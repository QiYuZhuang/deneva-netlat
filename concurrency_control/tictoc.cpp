...
#if OCC_WAW_LOCK
    if (ISOLATION_LEVEL == SERIALIZABLE) rc = validate_read_set(rset, txn, txn->_min_commit_ts);
#else
    if (rc == RCOK) {
...
RC Tictoc::validate_part(TxnManager *txn) {
    ...
    if (rc != Abort) {
        if (validate_write_set(wset, txn, txn->_min_commit_ts) == Abort) rc = Abort;
    }
    if (rc != Abort) {
...
void Tictoc::compute_commit_ts(TxnManager *txn) {
    ...
    for (uint64_t i = 0; i < size; i++) {
        if (txn->get_access_type(i) == WR) {
            txn->_min_commit_ts = txn->txn->accesses[i]->orig_rts + 1 > txn->_min_commit_ts
                                      ? txn->txn->accesses[i]->orig_rts + 1
                                      : txn->_min_commit_ts;
        } else {
            txn->_min_commit_ts = txn->txn->accesses[i]->orig_wts > txn->_min_commit_ts
                                      ? txn->txn->accesses[i]->orig_wts
                                      : txn->_min_commit_ts;
        }
    }
#endif
...
RC Tictoc::validate_read_set(tictoc_set_ent *rset, TxnManager *txn, uint64_t commit_ts) {
#if CC_ALG == TICTOC
#if 0
    for (auto access : _index_access_set) {
    ...
    for (UInt32 i = 0; i < rset->set_size; i++) {
        ...
        if (!cur_rrow->manager->try_renew(rset->access[i]->orig_wts, commit_ts,
                                          rset->access[i]->orig_rts)) {
            return Abort;
        }
    }
#endif
...
RC Tictoc::lock_write_set(tictoc_set_ent *wset, TxnManager *txn) {
    assert(!OCC_WAW_LOCK);
    RC rc = RCOK;
#if CC_ALG == TICTOC
    ...
    for (UInt32 i = 0; i < wset->set_size; i++) {
        ...
        if (rc == WAIT || rc == RCOK) wset->access[i]->locked = true;
        if (rc == WAIT) ATOM_ADD_FETCH(txn->_num_lock_waits, 1);
...
void Tictoc::cleanup(RC rc, TxnManager *txn) {
#if CC_ALG == TICTOC
    tictoc_set_ent *wset;
    tictoc_set_ent *rset;
    get_rw_set(txn, rset, wset);
...
void Tictoc::unlock_write_set(RC rc, tictoc_set_ent *wset, TxnManager *txn) {
#if CC_ALG == TICTOC
#if 0
...
RC Tictoc::validate_write_set(tictoc_set_ent *wset, TxnManager *txn, uint64_t commit_ts) {
#if CC_ALG == TICTOC
    for (UInt32 i = 0; i < wset->set_size; i++) {
...
bool Tictoc::is_txn_ready(TxnManager *txn) { return txn->_num_lock_waits == 0; }

void Tictoc::set_txn_ready(RC rc, TxnManager *txn) {

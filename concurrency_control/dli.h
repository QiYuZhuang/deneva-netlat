...
public:
    ...
    static TSNode<T>* push_front(std::atomic<TSNode<T>*>& head, Args&&... args) {
        ...
        while (!head.compare_exchange_weak(new_head->next_, new_head, std::memory_order_release,
...
public:
    ...
    struct ValidatedTxn {
        ...
        ValidatedTxn(const RWSet& rset, const RWSet& wset, const ts_t start_ts,
...
#else
        ValidatedTxn(const RWSet& rset, const RWSet& wset, const ts_t start_ts)
            : is_abort_(false), rset_(rset), wset_(wset), start_ts_(start_ts) {}
#endif
    ...
    static void get_rw_set(TxnManager* txn, RWSet& rset, RWSet& wset);
    void latch();
    void release();

...
void ssi::init() { sem_init(&_semaphore, 0, 1); }
...
uint64_t InOutTable::hash(uint64_t key) { return key % table_size; }
...
void InOutTable::down_inConflict(uint64_t thd_id, uint64_t key, uint64_t value) {
    ...
    if (idx != v_idx) pthread_mutex_lock(&table[idx].mtx);
    InOutTableEntry* entry = find(key);
    if (entry) {
        entry->inConflict.erase(value);
    }
    if (idx != v_idx) pthread_mutex_unlock(&table[idx].mtx);
}

...
bool Row_tictoc::CompareWait::operator()(TxnManager *en1, TxnManager *en2) const {
...
void Row_tictoc::latch() { pthread_mutex_lock(_latch); }

void Row_tictoc::unlatch() { pthread_mutex_unlock(_latch); }

RC Row_tictoc::access(access_t type, TxnManager *txn, row_t *&row, uint64_t &wts, uint64_t &rts) {
...
RC Row_tictoc::read(TxnManager *txn, char *data, uint64_t &wts, uint64_t &rts, bool latch,
                    bool remote) {
    uint64_t starttime = get_sys_clock();
    if (latch) this->latch();
    ...
    rts = _rts;
    // txn->cur_row = _row;
    if (data) memcpy(data, _row->get_data(), _row->get_tuple_size());
...
RC Row_tictoc::write(TxnManager *txn, uint64_t &wts, uint64_t &rts, bool latch) {
    RC rc = RCOK;
    assert(OCC_WAW_LOCK);
#if OCC_LOCK_TYPE == NO_WAIT
    if (_ts_lock) return Abort;
#endif
    uint64_t starttime = get_sys_clock();
    if (latch) pthread_mutex_lock(_latch);
    INC_STATS(txn->get_thd_id(), trans_access_lock_wait_time, get_sys_clock() - starttime);
    if (!_ts_lock) {
        _ts_lock = true;
        _lock_owner = txn;
        ...
        // 			"txn=%ld, _lock_owner=%ld. ID=%ld\n", (uint64_t)txn, (uint64_t)_lock_owner,
        // txn->get_txn_id()); txn has higher priority, should wait. assert
        ...
        if (_waiting_set.size() < _max_num_waits &&
            ...
            rc = Abort;
            DEBUG("tictoc abort 158 %ld,%lu -- %ld\n", txn->get_txn_id(), txn->_min_commit_ts,
    ...
    if (latch) pthread_mutex_unlock(_latch);
    return rc;
}

RC Row_tictoc::abort(access_t type, TxnManager *txn) {
...
RC Row_tictoc::commit(access_t type, TxnManager *txn, row_t *data) {
    uint64_t mtx_wait_starttime = get_sys_clock();
    // while(!ATOM_CAS(wkdb_avail,true,false)) { }
    INC_STATS(txn->get_thd_id(), mtx[33], get_sys_clock() - mtx_wait_starttime);
    DEBUG("tictoc Commit %ld: %d,%lu -- %ld\n", txn->get_txn_id(), type, txn->_min_commit_ts,
          _row->get_primary_key());

    uint64_t wts = txn->_min_commit_ts;
    latch();
    if (_deleted) assert(wts < _delete_timestamp);
    _wts = wts;
    _rts = wts;
    _row->copy(data);
    unlatch();
...
RC Row_tictoc::try_lock(TxnManager *txn) {
#if LOCK_ALL_DEBUG
...
bool Row_tictoc::try_renew(ts_t wts, ts_t rts, ts_t &new_rts) {
    if (wts != _wts) {
        if (_wts < rts) {
    ...
    if (_ts_lock) {
        ...
        // should check the lower bound of upcoming wts. Maybe we can still renew the current rts
        // without hurting the upcoming write.
        pthread_mutex_unlock(_latch);
        return false;
    }
...
void Row_tictoc::release(TxnManager *txn, RC rc) {
#if CC_ALG == TICTOC
    pthread_mutex_lock(_latch);
#if !OCC_WAW_LOCK
#if OCC_LOCK_TYPE == NO_WAIT
    _ts_lock = false;
#elif OCC_LOCK_TYPE == WAIT_DIE
    if (rc == RCOK) assert(_ts_lock && txn == _lock_owner);
    if (txn == _lock_owner) {
...
void Row_tictoc::get_ts(uint64_t &wts, uint64_t &rts) {
#if CC_ALG == TICTOC
    pthread_mutex_lock(_latch);
    wts = _wts;
    rts = _rts;
...
bool Row_tictoc::renew(ts_t wts, ts_t rts, ts_t &new_rts)  // Renew without lock checking
{
#if LOCK_ALL_BEFORE_COMMIT
#if ATOMIC_WORD
    uint64_t v = _ts_word;
    uint64_t lock_mask = (WRITE_PERMISSION_LOCK) ? WRITE_BIT : LOCK_BIT;
    if ((v & WTS_MASK) == wts && ((v & RTS_MASK) >> WTS_LEN) >= rts - wts) return true;
    if (v & lock_mask) return false;
#if TICTOC_MV
    COMPILER_BARRIER
...
#else
    if (wts != (v & WTS_MASK)) return false;
#endif
...
#else
#if TICTOC_MV
    if (wts < _hist_wts) return false;
#else
    if (wts != _wts) {
        if (_wts > rts + 1) INC_INT_STATS(int_possibMVCC, 1);
        return false;
    }
#endif
    pthread_mutex_lock(_latch);
    ...
    if (_row) {
        ...
        if (max_rts > _rts) new_rts = _rts = max_rts;
#endif
    }
#endif

...
AdaptorStat adaptor::get_state() { return state_; }

int64_t adaptor::get_interval(row_t *cur_w) { return time_interval_[cur_w->conflict_level]; }
...
RC adaptor::adaptor_cal() {
    ...
    for (int i = 1; i < LEVEL_COUNT; i++) {
        ...
        // 1. SA
        init_SA_Params(1, 1.5, 1, 0.01, 2 /* 2 epochs in fix T */, (unsigned)time(NULL) /* seed */,
                       5 * i /* max interval */);
        solve(i);
...
RC adaptor::run() {
    ...
    while (!simulation->is_done()) {
        ...
        // abort_count_); printf("[INFO] (zhuang).8 . start_time: %lu, end_time: %lu\n", start_time,
        // end_time); fflush(stdout); double a_ratio = energy();
        if (energy() > MAX_ABORT) {
            // TODO (zhuang): modify the time of thread sleep
            // printf("[INFO] (zhuang).1 sleep time. sleep time: %u, abort ratio: %f\n",
            // g_adaptor_sleep_time, a_ratio); fflush(stdout);
...
void adaptor::init_SA_Params(double t_initial, double t_denominator, double k, double t_min,
                             int32_t epochs_fixed_T, int64_t seed, int32_t max_interval) {
...
void adaptor::save_best(int32_t level) { best_interval_ = time_interval_[level]; }
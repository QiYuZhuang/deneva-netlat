...
void Row_ssi::insert_history(ts_t ts, TxnManager *txn, row_t *row) {
    ...
    if (row != NULL)
        ...
        rhis_len++;
    SSIHisEntry **queue = (row == NULL) ? &(readhis) : &(writehis);
    SSIHisEntry **tail = (row == NULL) ? &(readhistail) : &(writehistail);
...
SSILockEntry *Row_ssi::get_entry() {
    SSILockEntry *entry = (SSILockEntry *)mem_allocator.alloc(sizeof(SSILockEntry));
...
void Row_ssi::get_lock(lock_t type, TxnManager *txn) {
    ...
    if (type == LOCK_SH) STACK_PUSH(si_read_lock, entry);
    if (type == LOCK_EX) STACK_PUSH(write_lock, entry);
}
...
RC Row_ssi::access(TxnManager *txn, TsType type, row_t *row) {
    ...
    if (type == R_REQ) {
        ...
        while (whis != NULL && whis->ts > start_ts) whis = whis->next;
        ...
        while (whis != NULL && whis->ts > start_ts) {
            ...
            // if (out) {
            // 	DEBUG("ssi txn %ld read the write_commit out %ld abort, whis_ts %ld current_start_ts
            // %ld\n",txnid, whis->txn, whis->ts, start_ts); 	rc = Abort; 	goto end;
            // }
            whis = whis->next;
        }
        // Add RW dependencies
        ...
        while (si_read != NULL) {
            ...
            if (inout_table.get_state(txn->get_thd_id(), si_read->txn) == SSI_COMMITTED &&
                ...
                if (in) {
                    rc = Abort;
                    DEBUG(
                        "ssi txn %ld write the read_commit in %ld abort, rhis_ts %ld "
                        "current_start_ts %ld\n",
                        txnid, si_read->txn,
                        inout_table.get_commit_ts(txn->get_thd_id(), si_read->txn), start_ts);
                    goto end;
        ...
        // 	if (in) {
        // 		rc = Abort;
        // 		DEBUG("ssi txn %ld write the read_commit in %ld abort, rhis_ts %ld current_start_ts
        // %ld\n",txn->get_txn_id(), rhis->txn, rhis->ts, start_ts); 		goto end;
        // 	}
        // 	rhis = rhis->next;
        // }
        // rhis = readhis;

...
void wsi::init() { sem_init(&_semaphore, 0, 1); }
...
RC wsi::central_validate(TxnManager *txn) {
    ...
    if (!readonly) {
        for (UInt32 i = 0; i < rset->set_size; i++) {
            checked++;
#if CC_ALG == WSI
            if (rset->rows[i]->manager->get_last_commit() > start_tn) rc = Abort;
#endif
        }
    }
    // INC_STATS(txn->get_thd_id(),wsi_validate_time,get_sys_clock() - starttime);
...
void wsi::finish(RC rc, TxnManager *txn) { central_finish(rc, txn); }

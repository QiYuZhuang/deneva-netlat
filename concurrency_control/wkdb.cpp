...
void Wkdb::init() { sem_init(&_semaphore, 0, 1); }
...
RC Wkdb::validate(TxnManager* txn) {
    ...
    for (UInt32 i = 0; i < wset->set_size; i++) {
        ...
        for (auto it = readxid_list->begin(); it != readxid_list->end(); it++) {
            ...
            uint64_t txn_id = *it;
            DEBUG("    UR %ld -- %ld: %ld\n", txn->get_txn_id(), cur_wrow->get_primary_key(),
                  txn_id);
            ...
            if (state == WKDB_VALIDATED || state == WKDB_COMMITTED) {
                ...
                if (lower <= it_lower) {
                    ...
                    // running_txn_id.upper:%lu", ctx, part_ctx->GetTransID(), lower, upper,
                    // dta_txn->lower, dta_txn->upper);
                    lower = it_lower + 1;
...
uint64_t WkdbTimeTable::hash(uint64_t key) { return key % table_size; }

...
void dli_merge_set(Dli::RWSet& r_tc, Dli::RWSet& r_tl) { r_tc.insert(r_tl.begin(), r_tl.end()); }

void dli_compare_set(const Dli::RWSet& r_tc, uint32_t& newer_tc, const Dli::RWSet& r_tl,
                     uint32_t& newer_tl, bool rw, const uint32_t offset) {
...
static bool check_conflict(TxnManager* txn, Dli::RWSet& r_tc, Dli::RWSet& w_tc, Dli::RWSet& r_tl,
                           Dli::RWSet& w_tl) {
...
static bool check_conflict(TxnManager* txn, Dli::RWSet& r_tc, Dli::RWSet& w_tc, Dli::RWSet& r_tl,
                           Dli::RWSet& w_tl, uint64_t& lower, uint64_t& upper, uint64_t lower_tl,
                           uint64_t upper_tl) {
    uint32_t newer_tl = 0, newer_tc = 0;
...
static RC validate_main(TxnManager* txn, Dli* dli, const bool final_validate) {
    RC rc = RCOK;
#if CC_ALG == DLI_MVCC_OCC || CC_ALG == DLI_BASE || CC_ALG == DLI_OCC || CC_ALG == DLI_DTA || \
    CC_ALG == DLI_DTA2 || CC_ALG == DLI_DTA3 || CC_ALG == DLI_MVCC
        ...
        for (auto& i : wset) {
            if (i.first->manager->w_trans != ts) {
                if (!i.first->manager->w_trans.compare_exchange_weak(expect, ts)) {
                    /*compare_exchange_strong：if w_trans' value == expected. Then w_trans' value =
                      ts else expect=w_trans' value #Q: is there means? if P_REQ, row is ok, but its
                      version is 0. Once real write, set version. So w_trans' value != 0, then abort
                    */
                    rc = Abort;
                    break;
                }
...
#if CC_ALG == DLI_DTA || CC_ALG == DLI_DTA2 || CC_ALG == DLI_DTA3
            res = check_conflict(txn, rset, wset, tl->rset_, tl->wset_, lower, upper, tl->lower_,
                                 tl->upper_);
    ...
    if (rc == RCOK && final_validate) {
        ...
        txn->is_abort = &TSNode<Dli::ValidatedTxn>::push_front(dli->validated_txns_,
                                                               std::move(rset), std::move(wset), ts)
                             ->is_abort_;
        INC_STATS(txn->get_thd_id(), dli_push_front_time, get_sys_clock() - push_front_start_time);
#else
        uint64_t push_front_start_time = get_sys_clock();
        txn->is_abort =
            &TSNode<Dli::ValidatedTxn>::push_front(dli->validated_txns_, std::move(rset),
                                                   std::move(wset), ts, lower, upper)
                 ->is_abort_;
        INC_STATS(txn->get_thd_id(), dli_push_front_time, get_sys_clock() - push_front_start_time);

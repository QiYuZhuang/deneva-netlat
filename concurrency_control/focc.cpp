...
void Focc::finish(RC rc, TxnManager *txn) { central_finish(rc, txn); }
...
RC Focc::central_validate(TxnManager *txn) {
    ...
    if (!readonly) {
        ...
        for (ent = active; ent != NULL; ent = ent->next) {
            ...
            if (!valid) {
                INC_STATS(txn->get_thd_id(), occ_act_validate_fail_time,

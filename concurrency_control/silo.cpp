...
RC TxnManager::finish(RC rc) {
    if (rc == Abort) {
        if (this->num_locks > get_access_cnt()) return rc;
        ...
        for (uint64_t i = 0; i < txn->write_cnt; i++) {
            Access* access = txn->accesses[write_set[i]];
            access->orig_row->manager->write(access->data, this->commit_timestamp);
            txn->accesses[write_set[i]]->orig_row->manager->release();
...
RC TxnManager::find_tid_silo(ts_t max_tid) {
    if (max_tid > _cur_tid) _cur_tid = max_tid;
    return RCOK;
}

...
public:
    ...
    virtual void DeliverActionSequences(
        const std::function<void(ActionSequence &&, uint64_t)> &handle) const = 0;
};
...
public:
    ...
    void DeliverActionSequences(
        const std::function<void(ActionSequence &&, uint64_t)> &handle) const;
    static uint64_t cut_down_;

private:
    void HandleActionSeq(const std::function<void(ActionSequence &&, uint64_t)> &handle,
                         uint64_t seq_id, const ActionSequence &act_seq) const;
    void HandleDTLActionSeq(const std::function<void(ActionSequence &&, uint64_t)> &handle,
                            uint64_t seq_id, const ActionSequence &dml_act_seq,
                            const ActionSequence &dtl_act_seq) const;
    void HandleDMLActionSeq(const std::function<void(ActionSequence &&, uint64_t)> &handle,
                            uint64_t seq_id, const ActionSequence &dml_act_seq,

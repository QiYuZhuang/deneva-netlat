...
std::set<uint64_t> YCSBQuery::participants(Message* msg, Workload* wl) {
    ...
    for (uint64_t i = 0; i < ycsb_msg->requests.size(); i++) {
        uint64_t req_nid =
            GET_NODE_ID(((YCSBWorkload*)wl)->key_to_part(ycsb_msg->requests[i]->key));
    ...
    for (UInt32 i = 0; i < g_req_per_query; i++) {
        ...
        if (FIRST_PART_LOCAL && rid == 0) {
            if (hot < g_access_perc) {
                row_id = (uint64_t)(mrand->next() % (hot_key_max / g_part_cnt)) * g_part_cnt +
                         home_partition_id;
            } else {
            ...
            while (1) {
                if (hot < g_access_perc) {
                    ...
                    row_id = ((uint64_t)(mrand->next() % (g_synth_table_size - hot_key_max))) +
                             hot_key_max;
                }
                partition_id = row_id % g_part_cnt;
        ...
        if (FIRST_PART_LOCAL && rid == 0) {
            ...
            if (r_mpt < g_mpr) {
                ...
                if (rid == 1 && g_part_cnt > 1) {
                    while (partition_id ==
                           home_partition_id) {  // we need to assure that txn access remote

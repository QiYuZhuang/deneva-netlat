...
void TraversalActionSequenceCreator::DeliverActionSequences(
    const std::function<void(ActionSequence &&, uint64_t)> &handle) const {
    std::vector<Action> tmp_actions;
    RecursiveFillDMLActionSeq(
        [this, &handle](const ActionSequence &dml_act_seq, uint64_t seq_id,
                        const uint64_t max_trans_id) {
            HandleDMLActionSeq(handle, seq_id, dml_act_seq, max_trans_id);
        },
        tmp_actions, 0, 0);
}

void TraversalActionSequenceCreator::HandleActionSeq(
    const std::function<void(ActionSequence &&, uint64_t)> &handle, uint64_t seq_id,
...
void TraversalActionSequenceCreator::HandleDTLActionSeq(
    const std::function<void(ActionSequence &&, uint64_t)> &handle, uint64_t seq_id,
    const ActionSequence &dml_act_seq, const ActionSequence &dtl_act_seq) const {
    ...
    RecursiveMoveForwardDTLAction(
        [this, &handle, seq_id](const ActionSequence &act_seq) {
            HandleActionSeq(handle, seq_id, act_seq);
        },
        act_seq_tot, dml_act_seq.size());
}
void TraversalActionSequenceCreator::HandleDMLActionSeq(
    const std::function<void(ActionSequence &&, uint64_t)> &handle, uint64_t seq_id,
            ...
            for (uint64_t item_id = 0; item_id < std::min(max_item_id + 1, item_num_); ++item_id)
                for (Action::Type dml_act_type : {Action::Type::READ, Action::Type::WRITE}) {
                    ...
                    if (cur > 0 && dml_act_type == Action::Type::READ &&
                        ...
                        trans_id == actions[cur - 1].trans_id() &&
                        item_id == actions[cur - 1].item_id()) {
                        continue;
                    }
                    actions.emplace_back(dml_act_type, trans_id, item_id);
                    // if (!SkipDMLAction(actions)) { RecursiveFillDMLActionSeq(handle, actions,
                    // versions);
                    // }
 ...
 *
 ****************************************************************************/
void TraversalActionSequenceCreator::RecursiveFillDTLActionSeq(
    const std::function<void(const ActionSequence &)> &handle,
    std::vector<Action::Type> &dtl_act_types, const uint64_t max_trans_id,
...
void TraversalActionSequenceCreator::RecursiveMoveForwardDTLAction(
    const std::function<void(const ActionSequence &)> &handle, ActionSequence &act_seq,

...
void DAQuery::init(uint64_t thd_id, Workload* h_wl) { BaseQuery::init(); }

void DAQuery::init() { BaseQuery::init(); }

void DAQuery::release() { BaseQuery::release(); }

void DAQuery::print() {
    std::cout << "trans_id:" << trans_id
              << " "
                 "item_id"
              << item_id
              << " "
...
bool DAQuery::readonly() { return false; }
...
uint64_t DAQueryGenerator::action_2_state(ActionSequence& act_seq, size_t i, uint64_t seq_id) {
    ...
    if (i < 0 || i >= act_seq.actions().size()) return ret;
    trans_id2 = act_seq.actions()[i].trans_id();
    if (act_seq.actions()[i].IsPointDML()) item_id2 = act_seq.actions()[i].item_id();
...
BaseQuery* DAQueryGenerator::create_query(Workload* h_wl, uint64_t home_partition_id) {
    ...
    if (opt.subtask_num > 1) opt.subtask_id = home_partition_id;
    opt.max_dml = MAX_DML;
    ...
    const auto handle = [this](ActionSequence&& act_seq, uint64_t seq_id) {
        ...
        for (size_t i = 0; i < seq_size; i++) {
            ...
            if (act_seq.actions()[i].type() == Action::Type::WRITE) t_version[DAQ_t->item_id]++;

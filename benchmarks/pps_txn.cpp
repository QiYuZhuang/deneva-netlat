...
RC PPSTxnManager::run_txn() {
    ...
    if (IS_LOCAL(txn->txn_id) && (state == PPS_GETPART0 || state == PPS_GETPRODUCT0 ||
                                  state == PPS_GETSUPPLIER0 || state == PPS_GETPARTBYSUPPLIER0 ||
                                  state == PPS_GETPARTBYPRODUCT0 || state == PPS_ORDERPRODUCT0 ||
                                  state == PPS_UPDATEPRODUCTPART0 || state == PPS_UPDATEPART0)) {
...
inline RC PPSTxnManager::run_getpartsbyproduct_2(uint64_t product_key, row_t *&r_local) {
    ...
    item =
        index_read(index, product_key, products_to_partition(product_key), parts_processed_count);
    if (item == NULL) {
        r_local = NULL;
        return RCOK;
    }

...
void fake_copy_to_buf(Message* msg, char* buf) {
    ...
    *(double*)(buf + ptr) = msg->lat_process_time;
    ...
    if ((CC_ALG == CALVIN && (msg->rtype == CL_QRY || msg->rtype == CL_QRY_O) &&
         msg->txn_id % g_node_cnt == g_node_id) ||
        (CC_ALG != CALVIN && IS_LOCAL(msg->txn_id))) {
        msg->lat_network_time = get_sys_clock();
    } else {
        msg->lat_other_time = get_sys_clock() - msg->lat_other_time;
...
void MessageThread::run() {
  ...
  if (dest_node_id == g_node_id &&
    ...
    if (false) {
        ...
        if (msg->get_rtype() == RQRY || msg->get_rtype() == RQRY_RSP) {
        ...
        DEBUG("%ld enqueue Msg into workqueue %d, (%ld,%ld) to %ld\n", _thd_id, new_msg->rtype,
              new_msg->txn_id, new_msg->batch_id, dest_node_id);
        if (buffer[dest_node_id]->ready()) {
            assert(sbuf->cnt > 0);
            sbuf->reset(dest_node_id);
        }
        ...
        DEBUG("%ld enqueue Msg into workqueue %d, (%ld,%ld) to %ld\n", _thd_id, new_msg->rtype,
              new_msg->txn_id, new_msg->batch_id, dest_node_id);
        INC_STATS(0, trans_msgsend_stage_one,
                  get_sys_clock() - starttime - stage3_span1 - stage3_span2);
  ...
  assert(dest_node_id != g_node_id);
    ...
    if (!is_server && true)
        printf(
            "cl seq_id:%lu type:%c trans_id:%lu item:%c state:%lu next_state:%lu "
            "write_version:%lu\n",
            ...
            ((DAClientQueryMessage *)msg)->state, (((DAClientQueryMessage *)msg)->next_state),

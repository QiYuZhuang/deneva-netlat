...
void Client_query_queue::init(Workload* h_wl) {
    _wl = h_wl;
...
void* Client_query_queue::initQueriesHelper(void* args) {
    ((Client_query_queue*)((FUNC_ARGS*)args)->context)
        ->initQueriesParallel(((FUNC_ARGS*)args)->thd_id);

    return NULL;
}

void Client_query_queue::initQueriesParallel(uint64_t thd_id) {
#if WORKLOAD != DA
...
BaseQuery* Client_query_queue::get_next_query(uint64_t server_id, uint64_t thread_id) {
    ...
    uint64_t query_id = __sync_fetch_and_add(
        query_cnt[server_id], 1);  // return query_cnt[server_id]，then query_cnt[server_id]++
    if (query_id > g_max_txn_per_part) {
        __sync_bool_compare_and_swap(
            query_cnt[server_id], query_id + 1,
            0);  // if query_cnt[server_id]==query_id+1, then set query_cnt[server_id] to 0

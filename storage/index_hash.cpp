...
uint64_t IndexHash::get_count() { return _bucket_cnt; }
...
void BucketHeader::insert_item(idx_key_t key, itemid_t *item, int part_id) {
    ...
    if (cur_node == NULL) {
        BucketNode *new_node = (BucketNode *)mem_allocator.alloc(sizeof(BucketNode));
        new_node->init(key);
        new_node->items = item;
        if (prev_node != NULL) {
...
void BucketHeader::insert_item_nonunique(idx_key_t key, itemid_t *item, int part_id) {
    BucketNode *new_node = (BucketNode *)mem_allocator.alloc(sizeof(BucketNode));
    new_node->init(key);
    new_node->items = item;
    new_node->next = first_node;

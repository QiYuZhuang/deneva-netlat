...
#elif CC_ALG == DLI_MVCC_OCC || CC_ALG == DLI_DTA || CC_ALG == DLI_DTA2 || CC_ALG == DLI_DTA3 || \
    CC_ALG == DLI_MVCC
...
RC row_t::get_row(access_t type, TxnManager *txn, row_t *&row, uint64_t &orig_wts,
                  uint64_t &orig_rts) {
    RC rc = RCOK;
#if MODE == NOCC_MODE || MODE == QRY_ONLY_MODE
    row = this;
...
RC row_t::get_row_post_wait(access_t type, TxnManager *txn, row_t *&row) {
    ...
    assert(CC_ALG == WAIT_DIE || CC_ALG == MVCC || CC_ALG == WOOKONG || CC_ALG == TIMESTAMP ||
           CC_ALG == TICTOC || CC_ALG == SSI || CC_ALG == WSI || CC_ALG == DTA ||
           CC_ALG == DLI_DTA || CC_ALG == DLI_DTA2 || CC_ALG == DLI_DTA3 || CC_ALG == TIMESTAMP ||
           CC_ALG == TCM || CC_ALG == DNCC);
    ...
    CC_ALG == SSI || CC_ALG == WSI || CC_ALG == DTA || CC_ALG == DLI_DTA || CC_ALG == DLI_DTA2 || \
    CC_ALG == DLI_DTA3
    assert(txn->ts_ready);
    ...
    if ((CC_ALG == MVCC || CC_ALG == WOOKONG || CC_ALG == TICTOC || CC_ALG == SSI ||
         CC_ALG == WSI) &&
        type == WR) {

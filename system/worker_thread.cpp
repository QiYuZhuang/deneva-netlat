...
void WorkerThread::fakeprocess(Message *msg) {
    ...
    switch (msg->get_rtype()) {
        ...
        case RPREPARE:
            rc = RCOK;
            txn_man->set_rc(rc);
            msg_queue.enqueue(get_thd_id(), Message::create_message(txn_man, RACK_PREP),
                              msg->return_node_id);
            break;
        case RFWD:
            rc = process_rfwd(msg);
        ...
        case RQRY:
            rc = RCOK;
            txn_man->set_rc(rc);
            msg_queue.enqueue(get_thd_id(), Message::create_message(txn_man, RQRY_RSP),
                              msg->return_node_id);
            break;
        case RQRY_CONT:
            rc = RCOK;
        ...
        case RFIN:
            ...
            if (!((FinishMessage *)msg)->readonly || CC_ALG == MAAT || CC_ALG == OCC ||
                CC_ALG == TICTOC || CC_ALG == BOCC || CC_ALG == SSI)
                // if(!((FinishMessage*)msg)->readonly || CC_ALG == MAAT || CC_ALG == OCC)
...
void WorkerThread::commit() {
    ...
    uint64_t prepare_timespan =
        txn_man->txn_stats.finish_start_time - txn_man->txn_stats.prepare_start_time;
...
void WorkerThread::abort() {
    ...
    INC_STATS(get_thd_id(), trans_total_count, 1);
#if WORKLOAD != \
    DA  // actually DA do not need real abort. Just count it and do not send real abort msg.
    uint64_t penalty =
...
// #define TEST_MSG_order
        ...
        if ((msg->rtype != CL_QRY && msg->rtype != CL_QRY_O) || CC_ALG == CALVIN) {
            ...
            bool ready;
            if (msg->rtype == RFIN && !txn_man->finish_read_write)
                ready = false;
            else
...
RC WorkerThread::process_rfin(Message *msg) {
    ...
    if (!((FinishMessage *)msg)->readonly || CC_ALG == MAAT || CC_ALG == OCC || CC_ALG == TICTOC ||
        CC_ALG == BOCC || CC_ALG == SSI || CC_ALG == DLI_BASE || CC_ALG == DLI_OCC ||
        CC_ALG == SILO)
        msg_queue.enqueue(get_thd_id(), Message::create_message(txn_man, RACK_FIN),
                          GET_NODE_ID(msg->get_txn_id()));
    release_txn_man();
...
RC WorkerThread::process_rack_prep(Message *msg) {
    ...
    DEBUG("%ld bound set: [%ld,%ld] -> [%ld,%ld]\n", msg->get_txn_id(), lower, upper,
          wkdb_time_table.get_lower(get_thd_id(), msg->get_txn_id()),
          wkdb_time_table.get_upper(get_thd_id(), msg->get_txn_id()));
    if (((AckMessage *)msg)->rc != RCOK) {
    ...
    INC_STATS(get_thd_id(), trans_validation_network,
              get_sys_clock() - txn_man->txn_stats.trans_validate_network_start_time);
    // Done waiting
    ...
    if (rc == Abort || txn_man->get_rc() == Abort)
        txn_man->txn_stats.trans_abort_network_start_time = get_sys_clock();
    else
        txn_man->txn_stats.trans_commit_network_start_time = get_sys_clock();
...
RC WorkerThread::process_rack_rfin(Message *msg) {
    ...
    // if (!txn_man || !(txn_man->txn) || !txn_man->query ||
    // txn_man->query->partitions_touched.size() == 0 || txn_man->abort_cnt !=
    // msg->current_abort_cnt) {
    if (!txn_man || !(txn_man->txn) || !txn_man->query ||
        txn_man->query->partitions_touched.size() == 0) {
        DEBUG("RPREP_ACK skip %ld from %ld\n", msg->get_txn_id(), msg->get_return_id());
        return RCOK;
    ...
    if (txn_man->get_rc() == RCOK) {
        INC_STATS(get_thd_id(), trans_commit_network,
                  get_sys_clock() - txn_man->txn_stats.trans_commit_network_start_time);
        // txn_man->commit();
        commit();
    } else {
        INC_STATS(get_thd_id(), trans_abort_network,
                  get_sys_clock() - txn_man->txn_stats.trans_abort_network_start_time);
        // txn_man->abort();
        abort();
    }
    return rc;
...
RC WorkerThread::process_rqry_rsp(Message *msg) {
    ...
    // if (!txn_man->query || txn_man->query->partitions_touched.size() == 0 || txn_man->abort_cnt
    // != msg->current_abort_cnt) return RCOK;
...
#endif
#else
    INC_STATS(get_thd_id(), trans_process_network,
              get_sys_clock() - txn_man->txn_stats.trans_process_network_start_time);
    txn_man->txn_stats.remote_wait_time += get_sys_clock() - txn_man->txn_stats.wait_starttime;
...
// Only set new txn_id when txn first starts
    ...
    INC_STATS(get_thd_id(), trans_init_time,
              txn_man->txn_stats.init_complete_time - txn_man->txn_stats.restart_starttime);
    INC_STATS(get_thd_id(), trans_init_count, 1);
    if (rc != RCOK) return rc;
#if WORKLOAD == DA
    printf(
        "thd_id:%lu stxn_id:%lu batch_id:%lu seq_id:%lu type:%c rtype:%d trans_id:%lu item:%c "
        "laststate:%lu state:%lu next_state:%lu\n",
        this->_thd_id, ((DAClientQueryMessage *)msg)->txn_id,
        ((DAClientQueryMessage *)msg)->batch_id, ((DAClientQueryMessage *)msg)->seq_id,
        type2char(((DAClientQueryMessage *)msg)->txn_type), ((DAClientQueryMessage *)msg)->rtype,
        ((DAClientQueryMessage *)msg)->trans_id,
        static_cast<char>('x' + ((DAClientQueryMessage *)msg)->item_id),
        ((DAClientQueryMessage *)msg)->last_state, ((DAClientQueryMessage *)msg)->state,
        ((DAClientQueryMessage *)msg)->next_state);
    fflush(stdout);
#endif
    // Execute transaction
...
// btree
        ...
        // TODO (zhuang)
        // sleep(1);
        // printf("[INFO] (zhuang).10. sleep time: %u, high conflict: %d, normal conflict: %d, else
        // count: %d\n",

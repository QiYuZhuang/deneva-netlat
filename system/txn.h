...
public:
    ...
    row_t* get_access_original_row(uint64_t access_id) {
        return txn->accesses[access_id]->orig_row;
    }
...
#if CC_ALG == DLI_MVCC || CC_ALG == DLI_MVCC_OCC || CC_ALG == DLI_DTA || CC_ALG == DLI_DTA2 || \
    CC_ALG == DLI_DTA3 || CC_ALG == DLI_OCC

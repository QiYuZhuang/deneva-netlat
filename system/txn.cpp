...
#if CC_ALG != MAAT && CC_ALG != OCC && CC_ALG != WOOKONG && CC_ALG != TICTOC && CC_ALG != BOCC && \
    CC_ALG != FOCC && CC_ALG != DTA && CC_ALG != DLI_MVCC_OCC && CC_ALG != DLI_MVCC_BASE &&       \
    CC_ALG != DLI_DTA && CC_ALG != DLI_DTA2 && CC_ALG != DLI_DTA3 && CC_ALG != DLI_BASE &&        \
    CC_ALG != DLI_OCC
...
void TxnManager::send_finish_messages() {
    ...
    for (uint64_t i = 0; i < query->partitions_touched.size(); i++) {
        ...
        if (this->txn->rc == RCOK) assert(is_done());
...
uint64_t TxnManager::get_tcm_early() { return txn->early; }

void TxnManager::set_tcm_early(uint64_t early) { txn->early = early; }

uint64_t TxnManager::get_tcm_late() { return txn->late; }

void TxnManager::set_tcm_late(uint64_t late) { txn->late = late; }

bool TxnManager::get_tcm_end() { return txn->end; }

void TxnManager::set_tcm_end(bool end) { txn->end = end; }

bool TxnManager::get_tcm_committed() { return txn->committed; }

void TxnManager::set_tcm_committed(bool committed) { txn->committed = committed; }
...
#if ROLL_BACK && (CC_ALG == NO_WAIT || CC_ALG == WAIT_DIE || CC_ALG == DNCC || CC_ALG == HSTORE || \
                  CC_ALG == HSTORE_SPEC)
    if (type == WR) {
        // printf("free 10 %ld\n",get_txn_id());
...
#if CC_ALG == TICTOC
    if (_min_commit_ts > glob_manager.get_max_cts()) glob_manager.set_max_cts(_min_commit_ts);
#endif
...
RC TxnManager::get_row(row_t *row, access_t type, row_t *&row_rtn) {
    ...
    for (uint64_t i = 0; i < size; i++) {
        if (txn->accesses[i]->orig_row == row) {
            ...
            // access->orig_row, access->data, access->orig_rts, access->orig_wts);
            break;
        }
    }
    if (!access) {
    ...
    if (!OCC_WAW_LOCK || type == RD) {
        ...
        _min_commit_ts =
            _min_commit_ts > access->orig_rts + 1 ? _min_commit_ts : access->orig_rts + 1;
        access->locked = true;
    }
#else
    access_pool.get(get_thd_id(), access);
...
RC TxnManager::validate() {
    ...
    if (CC_ALG != OCC && CC_ALG != MAAT && CC_ALG != WOOKONG && CC_ALG != TICTOC &&
        CC_ALG != BOCC && CC_ALG != FOCC && CC_ALG != WSI && CC_ALG != SSI && CC_ALG != DLI_BASE &&
        CC_ALG != DLI_OCC && CC_ALG != DLI_MVCC_OCC && CC_ALG != DTA && CC_ALG != DLI_DTA &&
